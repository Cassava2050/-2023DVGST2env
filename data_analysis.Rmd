...
varcomp_root_rot <- 
  as.data.frame(obj$fitted_models$root_rot_number$`202394DVGST_momi`$mRand$root_rot_number$var.comp) %>% 
  mutate(across(where(is.double), round, 2)) %>% rownames_to_column("model_term")

colnames(varcomp_root_rot) <- c("model_term", "varcomp")
varcomp_root_rot %>% write.table("clipboard", col.names = T, row.names = F, sep = "\t")
```
...
```{r}
my_dat <- trial_rm_sd %>% 
  filter(!trial_name == "202395DVGST_ciat") %>% 
  add_column(block = NA) %>% mutate(block = as.factor(block))
...
met_results <- met_analysis(obj, 
                            filter_traits = 
                              trait_ideal[!trait_ideal %in% c("root_weight_commercial",
                                                              "root_rot_number"
                                                              )],
                            h2_filter = 0.1,
                            progress = TRUE
...
# bind the single estimates from CIAT
CIAT_BLUEs <- master_data$BLUP_BLUE_202395DVGST_ciat %>% select(genotype, starts_with("BLUEs_"))

CIAT_BLUEs_cq <- CIAT_BLUEs %>% select(genotype, BLUEs_WAB_20min_10mon:BLUEs_ave_WAB)

CIAT_BLUPs <- master_data$BLUP_BLUE_202395DVGST_ciat %>% select(genotype, starts_with("BLUPs_"))

CIAT_BLUPs_cq <- CIAT_BLUPs %>% select(genotype, BLUPs_WAB_20min_10mon:BLUPs_ave_WAB)
...
plot_and_save <- function(trait, file_name, met_results, trial_interest) {
  ...
  file_path <- paste0("images\\", file_name, "_vcor", trial_interest, ".png")
...
BLUEs_BLUPs <- BLUEs_BLUPs %>% 
  relocate(colnames(BLUEs_BLUPs)[str_detect(colnames(BLUEs_BLUPs), "DM_gravity|yield_ha")], .after = genotype)
...
names(master_data)[22] <- "BLUP_BLUE_root_commercial"
...
index_traits <- c("DM_gravity", "height_1st_branch", "yield_ha", "germination_perc")

index_dat <- blupDF_kp %>%
  select("accession_name", all_of(index_traits)) %>%
  filter(DM_gravity > 22, height_1st_branch > 80, yield_ha > 15, germination_perc > 80)
  drop_na()
```
...
res.pca <- pca_index(data = index_dat, id = "accession_name", b = c(10, 10, 15, 10), percentage = 0.60)
...
multiple.func <- function(x) {
  c(min = min(x, na.rm = T), max = max(x, na.rm = T), mean = mean(x, na.rm = T), median = median(x, na.rm = T))
}
  
sapply(selections[, c("DM_gravity", "height_1st_branch", "yield_ha", "germination_perc")], multiple.func)
```
...
grep("^BLUP_BLUE_root_number_commercial", names(master_data))
names(master_data)[23] <- "BLUP_BLUE_root_commercial"
