...
trial_standard_new <- trial_standard %>%
  mutate(obs_harvest_number_plan =
           case_when(str_detect(use_trial_name, "202393") ~ 4,
                     str_detect(use_trial_name, "202394") ~ 4,
                     str_detect(use_trial_name, "202395") ~ 2
                     ),
...
trial_standard_new <- trial_standard_new %>% mutate(obs_ave_WAB = rowMeans(select(trial_standard_new, obs_WAB_30min_10mon:obs_WAB_20min_10mon)))
# 
...
# Remove large DM_gravity

```{r}
outliers <- trial_tidy %>% filter(str_detect(trial_name, "202394"), DM_gravity > 50) %>% 
  select(plot_name, DM_gravity) %>% pull(plot_name)

trial_tidy <- trial_tidy %>% mutate(DM_gravity = ifelse(plot_name %in% outliers, NA, DM_gravity)) 
```
...
plot_bxp <- trial_tidy %>%
  ...
  filter(!var %in% c(
    ...
    "lodging1_3_6mon", "lodging1_3_4mon", "CBB_3mon"
  )) %>%
